...
const IncidentsViewTab = ({ initialCounters, ATMid }) => {
  ...
  const GetDemoIncidents = () => {
    const demoData = DataFile.DemoGetIncidentDetails.filter(incident => incident.Status === "Opened");
  ...
  const GetLiveIncidents = () => {
    ...
    apiRequest(
      "POST",
      "/IncidentController/GetIncidentsInfoAgainstATMID",
      {
        headers: {
       ...
       // console.log(response)
        if (response.error === "HTTP error! Status: 401") {
          ...
          if (response.data && response.data.length > 0) {
            swal(
              "Unable to get Incidents. Please try again later.",
              {
                buttons: {
                  Okay: true,
                },
      ...
      {loading && (
        <Box sx={{ width: "100%", backgroundColor: "#F9FAFB" }}>
          <p style={{ textAlign: "center", fontSize: "12px" }}>
            Fetching Incidents, Please Wait
          </p>
          {/* <LinearProgress /> */}
        </Box>
      )}
      ...
      >
        ...
        >
          ...
          return(
            ...
            <Grid item xs={3} key={incident.RepNumber} 
            sx={{
                display: "inline-flex",
                justifyContent: "center",
                alignItems: "center",
              ...
              variant="outlined"
                sx={{
                  ...
                  backgroundColor:value === index - 1 ? "#5F65FF0F !important" : "#FCFDFF",
...
export default IncidentsViewTab;
